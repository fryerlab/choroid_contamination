...
Read in files.
```{r}
counts <- read.delim("files/hippocampus_tpm_counts.txt", header = TRUE, sep = "\t")
gene.info <- read.delim("files/gene_info_for_counts_file.txt", header = TRUE, sep = "\t")
metadata <- read.csv("files/metadata.txt", header = TRUE)
```
...
for(i in 1:max){
  if (dirty_vs_clean$adj.P.Val[i] < 0.05){
    if (dirty_vs_clean$logFC[i] > 0.25){
      color_values <- c(color_values, 1) # 1 when logFC > 0.25 and pval < 0.05
    }
    else if (dirty_vs_clean$logFC[i] < -0.25){
      color_values <- c(color_values, 2) # 2 when logFC < -0.25 and pval < 0.05
    }
    else{
      color_values <- c(color_values, 3) # 3 when -0.25 =< logFC <= 0.25 pval < 0.05
    }
  }
  else{
...
dirty_vs_clean$color_p0.05_lfc0.25 <- factor(color_values)
```
...
p <-
  ggplot(data = dirty_vs_clean, 
         aes(x = logFC, # x-axis is logFC
             ...
             color = color_p0.05_lfc0.25)) + # color is based on factored Color column
  ...
  geom_vline(xintercept = -0.25,
             colour = "#000000",
             linetype = "dashed") +
  ggtitle("Dirty vs Clean Hippocampus\nFDRq < 0.05, -0.25 > LogFC > 0.25") +
  geom_text_repel(data = subset(dirty_vs_clean, dirty_vs_clean$color_p0.05_lfc0.25 == 1 |
                                  dirty_vs_clean$color_p0.05 == 2),

...
gene.info <- read.delim("geneInfo.txt", header = TRUE, sep = "\t")
metadata <- read.delim("hippocampus_metadata.txt", header = TRUE, sep = "\t")
```
...
Remove MT genes.  There are super abundant and can skew data.
```{r}
# annotation file provided does not have chr column
```
...
h3 <- ggplot(log2.ttr.counts, aes(x = log2.ttr.counts)) + 
  ...
  labs(title = "Allen Brain Atlas\nAging, Dementia and TBI\nHippocampus", 
       x=NULL, y=NULL) +
  xlab("log2(TTR TPM)") + ylab("# of Samples") + theme_bw() +
...
for(i in 1:max){
  if (dirty_vs_clean$adj.P.Val[i] < 0.05){
    if (dirty_vs_clean$logFC[i] > 1){
      ...
      color_values <- c(color_values, 3) # 3 when -1 =< logFC <= 1 and pval < 0.05
    }
  }
  else{

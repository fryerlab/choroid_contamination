...
```{r}
# read in counts
counts <- read.delim("Brain_Hippocampus", 
                     header = TRUE, 
                     sep = "\t",
                     row.names = 1)
# read in metadata
metadata <- read.delim("GTEx_Analysis_v8_Annotations_SubjectPhenotypesDS.txt",
                       header = TRUE,
                       sep = "\t")
...
pdf(
  "Figures/GTEx_all_histogram.pdf",
  width = 4, height = 4)
h4
```
...
point_shapes <- v$targets$sex # will say F or M for points
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste("DEGs/all_", i, "_FDRq1_Log2FC0.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste("DEGs/all_", i, "_FDRq0.05_Log2FC1.txt", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
...
dirty_vs_clean <-
  read.table(
    ...
    "DEGs/all_dirty_vs_clean_FDRq1_Log2FC0.txt",
    header = TRUE,
    sep = "\t",
    stringsAsFactors = F
  )
...
setwd("/Users/m214960/Documents/GTEx")
pdf(
  #"Figures/GTEx_volcano.pdf", 
  "Figures/GTEx_all_volcano.pdf",
  width = 4, height = 4)

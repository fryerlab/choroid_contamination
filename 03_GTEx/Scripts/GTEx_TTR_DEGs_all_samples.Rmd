...
h3 <- ggplot(log2.ttr.counts, aes(x = log2.ttr.counts)) + 
  geom_histogram(bins = 100, fill = "gray", color = "black") + 
  labs(title = "GTEx\nHippocampus (n = 197)", x=NULL, y=NULL) +
...
pdf(
  "Figures/GTEx_TTR_histogram.pdf",
  width = 4, height = 4)
h4
```
...
```{r}
#pdf("Figures/GTEx_TTR_MDS_voom.pdf", width = 4, height = 4)
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste("DEGs/DEGs_TTR_", i, "_FDRq1_Log2FC0.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste("DEGs/DEGs_TTR_", i, "_FDRq0.05_Log2FC1.txt", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
...
dirty_vs_clean <-
  read.table(
    "DEGs/DEGs_TTR_dirty_vs_clean_FDRq1_Log2FC0.txt",
    header = TRUE,
    sep = "\t",
    stringsAsFactors = F
  )
...
pdf("Figures/GTEx_TTR_volcano.pdf", width = 4, height = 4)
p
```
```{r age_violin}
TTR <- subset(dge.filtered.norm$counts, dge.filtered.norm$genes$gene_name == "TTR")
TTRmelt <- melt(TTR)

names(TTRmelt)[names(TTRmelt) == "Var2"] <- "subject_id"

df <- merge(TTRmelt, metadata, by = "subject_id")
df$age <- as.factor(df$age)
plot(df$age, log2(df$value))

age_violin <- ggplot(df, aes(x=age, y=log2(value))) + 
  geom_violin() + geom_boxplot(width=0.1, outlier.size = 0) +
  geom_jitter(shape=16, position=position_jitter(0.2)) +
  labs(
  title = "GTEx", # no main title
...
pdf("Figures/GTEx_age_violin.pdf", width = 6, height = 4)
age_violin

cor(as.numeric(df$age), log2(df$value),  method = "pearson")

df$age <- as.numeric(df$age)
df$value <- log2(df$value)
ggscatter(df, x = "age", y = "value", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Age bins", ylab = expression(log[2](italic(TTR))))

kruskal.test(df$value~df$age)

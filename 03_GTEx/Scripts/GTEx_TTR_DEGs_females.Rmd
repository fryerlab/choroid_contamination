...
h3 <- ggplot(log2.ttr.counts, aes(x = log2.ttr.counts)) + 
  geom_histogram(bins = 100, fill = "gray", color = "black") + 
  labs(title = "GTEx\nFemale Hippocampus (n = 54)", x=NULL, y=NULL) +
...
pdf(
  "Figures/GTEx_TTR_females_histogram.pdf",
  width = 4, height = 4)
h4
```
...
```{r}
#pdf("Figures/GTEx_TTR_females_MDS_voom.pdf", width = 4, height = 4)
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste("DEGs/DEGs_TTR_females_", i, "_FDRq1_Log2FC0.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste("DEGs/DEGs_TTR_females_", i, "_FDRq0.05_Log2FC1.txt", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
...
dirty_vs_clean <-
  read.table(
    "DEGs/DEGs_TTR_females_dirty_vs_clean_FDRq1_Log2FC0.txt",
    header = TRUE,
    sep = "\t",
    stringsAsFactors = F
  )
...
p <-
  ...
  geom_vline(xintercept = -1,
             colour = "#000000",
             linetype = "dashed") +
  ggtitle("GTEx Female TTR Groups\nFDRq < 0.05, -1 > LogFC > 1") +
  geom_text_repel(data = subset(dirty_vs_clean, 
                  ...
                  max.overlaps = getOption("ggrepel.max.overlaps", 
                                           default = 60)) +
  geom_text_repel(data = subset(dirty_vs_clean, 
                                dirty_vs_clean$gene_name == "FOLR1"),
                  aes(x = logFC, y= -log10(adj.P.Val), label = gene_name), 
                  color = "maroon",
                  fontface="italic") +
  geom_text_repel(data = subset(dirty_vs_clean,
...
```{r}
setwd("/Users/m214960/Documents/GTEx")
pdf("Figures/GTEx_TTR_females_volcano.pdf", width = 4, height = 4)
p
```

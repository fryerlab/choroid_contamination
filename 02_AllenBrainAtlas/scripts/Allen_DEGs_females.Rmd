...
setwd("/Users/m239830/Dropbox (ASU)/Fryer_Lab/TTR-V2-SUBMISSION/Allen_Brain_Atlas")
```
...
counts <- read.delim("Files/hippocampus_tpm_counts.txt", 
                     header = TRUE, 
                     sep = "\t")
gene.info <- read.delim("Files/geneInfo.txt", 
                        header = TRUE, 
                        sep = "\t")
metadata <- read.delim("Files/hippocampus_metadata.txt", 
                       header = TRUE, 
                       sep = "\t")
```
...
```{r}
keepFemale <- dge$samples$sex == "F"
dge <- dge[,keepFemale,keep.lib.sizes=FALSE]
dim(dge)
```
...
```{r, message=FALSE}
setwd("/Users/m239830/Dropbox (ASU)/Fryer_Lab/TTR-V2-SUBMISSION/Allen_Brain_Atlas/")
pdf("Allen_TTR_female_histogram.pdf", width = 4, height = 4)
h4
```
...
```{r}
#pdf("Figures/Allen_TTR_female_MDS_voom.pdf", width = 4, height = 4)
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste("DEGs/Allen_DEGs_TTR_female_", i, "_FDRq1_Log2FC0.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste("DEGs/Allen_DEGs_TTR_female_", i, "_FDRq0.05_Log2FC1.txt", sep = "") 
  write.table(
    vTopTable2,
    path,
    sep = "\t",
...
dirty_vs_clean <-
  read.table(
    "DEGs/Allen_DEGs_TTR_female_dirty_vs_clean_FDRq1_Log2FC0.txt",
    header = TRUE,
    sep = "\t",
    stringsAsFactors = F
  )
...
```{r}
max <- max(dirty_vs_clean$adj.P.Val[dirty_vs_clean$adj.P.Val < 0.05], 
           na.rm=TRUE)
hadjpval <- -log10(max)
hadjpval
```
...
```{r}
setwd("/Users/m239830/Dropbox (ASU)/Fryer_Lab/TTR-V2-SUBMISSION/Allen_Brain_Atlas/")
pdf("Figures/Allen_TTR_female_volcano.pdf", width = 4, height = 4)
p
dev.off()
```

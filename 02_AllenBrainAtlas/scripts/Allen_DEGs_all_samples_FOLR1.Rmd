...
h3 <- ggplot(log2.FOLR1.counts, aes(x = log2.FOLR1.counts)) + 
  ...
  labs(title = "Allen Brain Atlas\nHippocampus (n = 94)", 
       x=NULL, y=NULL) +
  labs(x = expression(log[2](italic(FOLR1))), y = "# of samples") +
  theme_bw() +
...
```{r, message=FALSE}
setwd("~/TTR/02_AllenBrainAtlas/")
pdf("Allen_FOLR1_histogram.pdf", width = 4, height = 4)
h4
```
...
```{r}
#pdf("Figures/Allen_FOLR1_MDS_voom.pdf", width = 4, height = 4)
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste("DEGs/Allen_DEGs_FOLR1_", i, "_FDRq1_Log2FC0.txt", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste("DEGs/Allen_DEGs_FOLR1_", i, "_FDRq0.05_Log2FC1.txt", sep = "") 
  write.table(
    vTopTable2,
    path,
    sep = "\t",
...
dirty_vs_clean <-
  read.table(
    "DEGs/Allen_DEGs_FOLR1_dirty_vs_clean_FDRq1_Log2FC0.txt",
    header = TRUE,
    sep = "\t",
    stringsAsFactors = F
  )
